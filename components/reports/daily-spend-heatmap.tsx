...
export function DailySpendHeatmap({ data, month, year, currency }: Props) {
  ...
  return (
    <div
      role="img"
      aria-label="Daily spending heatmap showing spending intensity throughout the month"
    >
      <div className="flex flex-col gap-3 sm:gap-4">
        ...
        <div className="grid grid-cols-7 gap-1 sm:gap-2">
          {dayNames.map((day, idx) => (
            <div
              key={day}
              className="text-center text-xs font-semibold text-text-secondary p-1 sm:p-2"
            >
              ...
              {week.map((day, dayIndex) => {
                ...
                return (
                  <div key={dayIndex}>
                    {day ? (
                      <div
                        className="relative aspect-square flex items-center justify-center rounded-lg text-xs sm:text-sm font-medium cursor-pointer transition-all duration-200 hover:ring-2 hover:ring-primary"
        ...
        {/* Legend */}
        <div className="flex flex-wrap items-center gap-2 sm:gap-3 text-xs mt-3 sm:mt-4">
          ...
          <div className="flex gap-1 items-center">
            <div
              className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded"
              style={{ backgroundColor: "var(--color-yellow-2)" }}
            />
          ...
          <div className="flex gap-1 items-center">
            <div
              className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded"
              style={{ backgroundColor: "var(--color-yellow-3)" }}
            />
            <span className="text-text-secondary text-xs">Med</span>
          </div>
          <div className="flex gap-1 items-center">
            <div
              className="w-2.5 h-2.5 sm:w-3 sm:h-3 rounded"
              style={{ backgroundColor: "var(--color-yellow-5)" }}
            />
            <span className="text-text-secondary text-xs">High</span>
          </div>
          <div className="flex gap-1 items-center">
            <div
              className="w-2.5 h-2.5 left-4 sm:w-3 sm:h-3 rounded"
              style={{ backgroundColor: "var(--color-primary)" }}
            />
            <span className="text-text-secondary text-xs">V.High</span>
          </div>
        </div>
      </div>
    </div>
