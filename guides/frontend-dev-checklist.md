...
- [x] Only shown to new users with zero categories
- [x] Step 1: Welcome screen with Finora branding + value prop
- [x] Step 2: Confirms default categories being seeded (shows list)
- [x] Step 3 (optional): Prompt for first income entry
- [x] "Get Started" / "Skip" on optional step → redirect to `/dashboard`
- [x] Glass card layout, yellow CTA buttons

### 10.2 Redirect logic

- [x] After login, check if `categories.length === 0` → redirect to `/onboarding`
- [x] Returning user with categories → straight to `/dashboard`

### 10.3 Exit criteria

- [x] New user sees onboarding and categories are seeded
- [x] Returning user bypasses onboarding
- [x] Skip button works
...
- [x] Above the fold: Finora logo + one-line value prop + "Sign In" + "Get Started" CTAs
- [x] Feature highlights (3–4 cards): Budget tracking, Savings goals, AI insights, Reports
- [x] Clean, professional — yellow primary accents on light/dark background
- [x] No auth required

### 11.2 Exit criteria

- [x] Renders with no session required
- [x] "Get Started" → `/register`; "Sign In" → `/login`
...
- [x] `app/(app)/error.tsx` — error boundary for protected routes (glass card + "Something went wrong" + retry button)
- [x] `app/(app)/loading.tsx` — loading UI for protected routes
- [x] All data-heavy pages have page-level skeleton loading states

### 12.2 Accessibility

- [x] Visible `:focus-visible` ring styles in `globals.css` (`outline: 2px solid var(--color-primary)`)
- [x] `prefers-reduced-motion` media query disables transitions/animations in `globals.css`
- [x] All interactive elements keyboard-navigable (Tab + Enter/Space)
- [x] All form inputs have associated `<label>` elements or `aria-label`
- [x] Color is never the only indicator (add icons or text labels alongside color meaning)
- [x] All images and chart visuals have `alt` text

### 12.3 Form UX

- [x] All Zod validation errors appear as field-level messages — not only as toasts
- [x] Submit buttons disabled while server action is in-flight (loading spinner icon)
- [x] Success: Sonner toast + redirect or form reset
- [x] Error: Sonner toast + field errors remain visible

### 12.4 Edge cases & zero states

- [x] No transactions → `empty-state.tsx` renders instead of empty table
- [x] Budget with no transactions → shows "₵0 / ₵500" (not null error)
- [x] Goal at 100% → "Completed" badge visible, contribution form hidden
- [x] AI Insights with < 1 month data → AI responds with a clear "insufficient data" message (no crash)
- [x] Reports month with no data → charts show zero lines / empty state
- [x] OTP expired (15 min) → inline error "Code expired — please request a new one" with "Resend code" button highlighted
- [x] OTP wrong → inline error "Incorrect code. Please try again" (not a toast — stays visible)
- [x] Resend cooldown active → "Resend code" button disabled with live countdown; shows `{ error }` message if cooldown triggered via API
- [x] Reset link expired (1 hour) → `/reset-password` page shows error state with "Get a new link" → `/forgot-password`
- [x] Reset link already consumed → same expired error state (backend returns identical error message — no information leakage)

### 12.5 Responsive layout audit

- [x] **375px (mobile):** Sidebar hidden → drawer/bottom-nav active; cards stack to single column; no horizontal scroll
- [x] **768px (tablet):** Sidebar icon-only; 2-column card grids
- [x] **1024px (desktop):** Full sidebar; multi-column layouts
- [x] **1440px (wide):** Max-width container; no stretched layouts

### 12.6 Pre-delivery visual QA checklist

- [x] Light mode QA pass across all pages
- [x] Dark mode QA pass across all pages
- [x] Mobile layout QA (375px viewport)
- [x] No broken glass card contrast in light mode (use `bg-white/80` minimum in light)
- [x] All Lucide icons consistent in size (`w-5 h-5` default, `w-4 h-4` small)
- [x] Currency formatting correct for GHS (₵), USD ($), GBP (£) at minimum
- [x] All hover states produce visible feedback
- [x] All loading skeletons match the shape of the content they replace (no layout shift)

### 12.7 Code quality

- [x] `npm run lint` — zero errors (2 warnings remain: React Hook Form incompatible library + unused error in categories catch)
- [x] `npm run build` — clean, no TypeScript errors
- [x] No `any` types in component props or hooks
- [x] No hard-coded hex values in any `.tsx` or `.css` file — all via CSS variables or Tailwind aliases
